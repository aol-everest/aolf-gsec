...
CLOUDFRONT_DISTRIBUTION_ID="E2O7PMT0VIGSF6"  # Set this if you have a CloudFront distribution
...
main() {
  ...
  # Deploy to S3
  log "Deploying to S3 bucket: $S3_BUCKET in path /uat/frontend/..."
  aws s3 sync build/ s3://$S3_BUCKET/uat/frontend/ --delete
  ...
  if aws s3 ls s3://$S3_BUCKET/uat/frontend/index.html &> /dev/null; then
  ...
  log "Frontend should be accessible at http://$S3_BUCKET.s3-website-us-east-2.amazonaws.com/uat/frontend/"
