...
COUNT=$(PGPASSWORD="$DB_PASSWORD" psql -h $DB_HOST -U $DB_USER -d $DB_NAME -t -c "SELECT COUNT(*) FROM ${SCHEMA}.geo_countries;")
...
if [ "$COUNT" -gt "0" ]; then
    ...
    PGPASSWORD="$DB_PASSWORD" psql -h $DB_HOST -U $DB_USER -d $DB_NAME -c "DELETE FROM ${SCHEMA}.geo_countries;"
fi
...
python3 "${SCRIPT_DIR}/20250604-load_countries-json_to_sql.py" "$JSON_FILE" > "$TEMP_SQL_FILE"
...
FINAL_COUNT=$(PGPASSWORD="$DB_PASSWORD" psql -h $DB_HOST -U $DB_USER -d $DB_NAME -t -c "SELECT COUNT(*) FROM ${SCHEMA}.geo_countries;")
