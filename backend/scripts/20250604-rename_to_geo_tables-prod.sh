...
# Script to rename tables to geo_ prefix in Production environment
# Usage: ./rename_to_geo_tables-prod.sh
...
echo "Renaming tables to geo_ prefix in PRODUCTION environment..."
echo "Host: $DB_HOST"
...
if [ $? -ne 0 ]; then
    echo "Error: Unable to connect to production database. Please check your database credentials."
    exit 1
fi
...
# Production safety: Create backup first
echo ""
echo "Step 1.5: Creating backup of existing tables..."
if [ "$countries_exists" = "t" ]; then
    echo "Creating backup of countries table..."
    PGPASSWORD="$DB_PASSWORD" psql -h $DB_HOST -U $DB_USER -d $DB_NAME -c "
        CREATE TABLE ${SCHEMA}.countries_backup_$(date +%Y%m%d) AS SELECT * FROM ${SCHEMA}.countries;
    "
fi

if [ "$subdivisions_exists" = "t" ]; then
    echo "Creating backup of subdivisions table..."
    PGPASSWORD="$DB_PASSWORD" psql -h $DB_HOST -U $DB_USER -d $DB_NAME -c "
        CREATE TABLE ${SCHEMA}.subdivisions_backup_$(date +%Y%m%d) AS SELECT * FROM ${SCHEMA}.subdivisions;
    "
fi
...
echo "- countries_backup_$(date +%Y%m%d)"
echo "- subdivisions_backup_$(date +%Y%m%d)" 